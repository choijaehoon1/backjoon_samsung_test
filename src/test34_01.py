...
while True:
    ...
    for i in range(2):
        for j in range(N):
            if i == 0 and j == N-1 and robot[i][j] != 0:
                # robot_list.append([i+1,j,robot[i][j]])
                robot[i][j] = 0
            if robot[i][j] != 0:
    ...
    for robo in robot_list[:]:
        ...
        if x == 1 and y == 0 and robot[x-1][y] == 0 and belt[x-1][y] >= 1:
            robot[x][y] = 0
            robot[x-1][y] = s
            belt[x-1][y] -= 1            
            robot_list.append([x-1,y,s])
            robot_list.remove([x,y,s])
        
        if x == 1 and y <= N-1 and robot[x][y-1] == 0 and belt[x][y-1] >= 1:
            robot[x][y] = 0
            robot[x][y-1] = s
            belt[x][y-1] -= 1            
            robot_list.append([x,y-1,s])
